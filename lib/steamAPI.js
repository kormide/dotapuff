...
function getPeers(steamApiKey, playerID, callback) {
    ...
    request('https://api.steampowered.com/IDOTA2Match_570/GetMatchHistory/v001/?' + querystring.stringify(params), function(error, response, body) {
        if (!error && response.statusCode == 200) {
            ...
            if (result.status === 1) {
                ...
                getPlayerSummaries(steamApiKey, players, function(result) {
                    if (result.success)
...
function getPlayerStats(steamApiKey, playerID, numMatches, callback) {
    getMatchHistory(steamApiKey, playerID, numMatches, function(response) {
        if (!response.success) {
...
function getMatchHistory(steamApiKey, playerID, numMatches, callback) {
    console.log("info [steam-api]: requesting match history (n=" + numMatches.toString() + ") for player " + playerID);
    getMatchHistoryRec(steamApiKey, playerID, numMatches, true, null, [], function(response) {
        if (response.success)
...
function getMatchHistoryRec(steamApiKey, playerID, numMatches, isFirstCall, startMatchID, allMatchIDs, callback) {
    var params = {
        key: steamApiKey,
        language: 'en_us',
        format: 'JSON',
        ...
        if (!error && response.statusCode == 200) {
            ...
            if (result.status === 1) {
                ...
                if (numMatches > 0 && result.matches.length === 25) {
                    ...
                    getMatchHistoryRec(steamApiKey, playerID, numMatches, false, startMatchID, allMatchIDs, callback);
                } else {
...
function getPlayerSummaries(steamApiKey, playerIDs, callback) {
