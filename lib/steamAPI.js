...
function getAllMatches(playerID, callback) {
...
function getPlayerStats(playerID, callback) {
    getAllMatches(playerID, function(response) {
        if (!response.success) {
            ...
            // **TEMP**: speed boost
            if (response.matches.length > 50) {
               response.matches = response.matches.slice(0, 50);
            }
